...
var yearScenes = [
	{
		...
		desktopDuration: 100,
		mobileDuration: 50,
		targetClass: ".story__y2009",
		classToggled: ".story__year--active",
	},
	{
		name: 'toggleYear2010',
		triggerElement: ".story__wrapper",
	...
	},
	{
		...
		desktopDuration: 150,
		mobileDuration: 30,
		targetClass: ".story__y2016",
		classToggled: ".story__year--active",
	},
]
console.log(yearScenes);
let desktopOffset = 50;
let mobileOffset = 0;
for (var i = 0 ; i < yearScenes.length; i++) {
	const el = yearScenes[i];
	if (getTypeOfMedia() === "large" || getTypeOfMedia() === "x-large") {
		console.log(desktopOffset*vh());
		new ScrollMagic.Scene({
			triggerElement: el.triggerElement,
			duration:el.desktopDuration*vh(),
			offset: desktopOffset*vh(),
			})
			.setClassToggle(el.targetClass, el.classToggled)
			.addTo(controller);
	} else {
		new ScrollMagic.Scene({
			triggerElement: el.triggerElement,
			duration:el.mobileDuration*vh(),
			offset: mobileOffset*vh()
			})
			.setClassToggle(el.targetClass, el.classToggled)
			.addTo(controller);
	}
	desktopOffset = desktopOffset + el.desktopDuration;
	mobileOffset = mobileOffset + el.mobileDuration;
}
